...
test("parachutist role tint and disposal stay local to one rig and leave face and dark cloth intact", () => {
  ...
  const darkAndFaceColors = new Map([...firstResources.materials]
  ...
  try {
    ...
    for (const [material, color] of darkAndFaceColors) assert.equal(material.color.getHex(), color);
