...
function wingBadge(group, add, silver, orange, x, y, z, scale = 1) {
  ...
  add(group, ellipsoid(x, y - 0.003 * scale, z, 0.0045 * scale, 0.009 * scale, 0.0015), orange);
}
...
export function createParachutistCharacter() {
  ...
  const fabric = new MeshStandardMaterial({ ...textile, color: 0x45494a, side: DoubleSide });
  const orange = new MeshStandardMaterial({ ...textile, color: 0xffa24e, side: DoubleSide });
  ...
  for (const side of [-1, 1]) {
    ...
    add(body, ribbon([[side * 0.18, 0.28, -0.069], [side * 0.20, 0.20, -0.057],
      ...
      [side * 0.137, -0.22, -0.057]], 0.025), orange);
  }

  // Pack has a convex cordura shell, an orange swept V and raised sewn panels.
  add(body, loft([
  ...
  for (const side of [-1, 1]) {
    add(body, ribbon([[side * 0.14, 0.31, 0.252], [side * 0.119, 0.20, 0.300],
      [side * 0.067, 0.045, 0.315], [side * 0.031, -0.092, 0.305]], 0.035), orange);
  ...
  wingBadge(body, add, silver, orange, 0, 0.11, 0.315, 1.1);
  ...
  add(headGroup, ribbon(hoodStripe, 0.020), orange);
  wingBadge(headGroup, add, silver, orange, -0.020, 0.098, -0.084, 0.45);
  ...
  for (const side of [-1, 1]) {
    ...
    add(lower, sleeveStripe([[-0.012, 0.052, 0.057], [-0.09, 0.051, 0.054, -0.002],
      [-0.18, 0.044, 0.047, -0.003], [-0.273, 0.037, 0.038, -0.004]], side), orange);
    ...
    wingBadge(lower, add, silver, orange, 0, -0.333, -0.039, 0.42);
    ...
    add(thigh, ribbon([[side * 0.085, -0.045, -0.024], [side * 0.093, -0.16, -0.025],
      [side * 0.078, -0.278, -0.024]], 0.015, [0, 0, 1]), orange);
    ...
    add(calf, ribbon([[side * 0.054, -0.021, -0.036], [side * 0.062, -0.074, -0.036],
      ...
      [side * 0.044, -0.365, -0.022]], 0.027, [0, 0, 1]), orange);
    ...
    wingBadge(boot, add, silver, orange, 0, 0.101, -0.054, 0.45);
  ...
  for (const [group, materials] of batches) {
    for (const [material, geometries] of materials) {
      ...
      mesh.name = `${group.name}-${material === orange ? "orange-panels" : "surface"}`;
      mesh.castShadow = true;
      mesh.receiveShadow = true;
      group.add(mesh);
    }
  }
  const roleSurfaces = [orange].map(surface => ({
    surface,
