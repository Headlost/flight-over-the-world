...
Secret `VITE_CESIUM_ION_KEY` przyjmuje jeden publiczny token odczytu albo całą kolejkę tokenów rozdzielonych przecinkami, z aktywnym na początku. Opcjonalna lokalna zmienna `VITE_CESIUM_ION_FALLBACK_KEYS` dopisuje dalsze tokeny do kolejki (obecny workflow przekazuje tylko `VITE_CESIUM_ION_KEY`). Gra przechodzi na kolejny wyłącznie po odpowiedzi Cesium ion 401 (nieważny lub cofnięty token). Nie przełącza kont po błędach uprawnień 403, limitu 402/429 ani błędach sieci. Kolejność obowiązuje w całej otwartej karcie, także po ponownym rozpoczęciu lotu; po ostatnim tokenie nie wraca do pierwszego. Kolejną planową rotację wykonuje właściciel: zmienia kolejność w `VITE_CESIUM_ION_KEY` i przebudowuje stronę. Samo zmienienie secretu nie aktualizuje opublikowanej gry. Bieżąca kolejność: KrurWer (aktywny), GoraM, PawelekMega, główny. Wartości tokenów pozostają w ignorowanym `.env.local` i GitHub Actions secret.
