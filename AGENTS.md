...
User-confirmed performance checkpoint on 2026-09-14: local Git tag local-known-good-2026-09-14-performance points to 533ad4611f009b737af4aeee9a4372354bc40bf9. Parameters, source snapshots and a source archive are saved in ignored .local-baselines/performance-2026-09-14/. Preserve this terrain sharpness and continuity when changing session admission. Numeric parameters alone do not restore the replacement-frontier and sky depth fixes.

The user subsequently confirmed zero usage and the full quota on all three supporter accounts. A first, one-time confirmedRemainingBatch may allocate at most the confirmed remaining amount per account across its entire lifetime, independently of unknown provider reset time. Its admissionStart/admissionUntil are operator validity dates, not claimed provider billing boundaries. The count must never reset automatically or be renewed by changing the snapshot after claims. Main remains blocked in this initial batch. Further monthly allocations require separately confirmed periods and reconciliation; direct use outside the workflow is still outside its visibility.
